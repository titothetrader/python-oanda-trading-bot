...
def get_test_pairs(pair_str):
    existing_pairs = oanda_instrument.Instrument.get_instruments_dict().keys()
...
def run():
    ...
    for pairname in test_pairs:
        ...
        i_pair = oanda_instrument.Instrument.get_instruments_dict()[pairname]

        h4_data = pd.read_pickle(oanda_utils.get_his_data_filename(pairname, "H4"))
        m5_data = pd.read_pickle(oanda_utils.get_his_data_filename(pairname, "M5"))
